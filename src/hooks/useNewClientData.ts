...
export const useNewClientData = () => {
  ...
  const fetchNewClientData = async () => {
    try {
      ...
      const newClientData: NewClientData[] = rows.slice(1).map((row: (string | number)[]) => ({
        memberId: String(row[0] || ''),
        firstName: String(row[1] || ''),
        lastName: String(row[2] || ''),
        email: String(row[3] || ''),
        phoneNumber: String(row[4] || ''),
        firstVisitDate: String(row[5] || ''),
        firstVisitEntityName: String(row[6] || ''),
        firstVisitType: String(row[7] || ''),
        firstVisitLocation: String(row[8] || ''),
        paymentMethod: String(row[9] || ''),
        membershipUsed: String(row[10] || ''),
        homeLocation: String(row[11] || ''),
        classNo: parseFloat(String(row[12])) || 0,
        trainerName: String(row[13] || ''),
        isNew: String(row[14] || ''),
        visitsPostTrial: parseFloat(String(row[15])) || 0,
        membershipsBoughtPostTrial: String(row[16] || ''),
        purchaseCountPostTrial: parseFloat(String(row[17])) || 0,
        ltv: parseFloat(String(row[18])) || 0,
        retentionStatus: String(row[19] || ''),
        conversionStatus: String(row[20] || ''),
        period: String(row[21] || ''),
        unique: String(row[22] || ''),
        firstPurchase: String(row[23] || ''),
        conversionSpan: parseFloat(String(row[24])) || 0,
      }));
