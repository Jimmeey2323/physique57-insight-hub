...
export const DiscountImpactInsights: React.FC<DiscountImpactInsightsProps> = ({ data }) => {
  const insights = useMemo(() => {
    ...
    const nonDiscountedItems = data.filter(item => (item.discountAmount || 0) === 0);

    const totalDiscounts = discountedItems.reduce((sum, item) => sum + (item.discountAmount || 0), 0);
    const totalRevenue = data.reduce((sum, item) => sum + (item.grossRevenue || 0), 0);
    
    const avgDiscountedOrderValue = discountedItems.length > 0 
      ? discountedItems.reduce((sum, item) => sum + (item.grossRevenue || 0), 0) / discountedItems.length 
      : 0;
    
    const avgNonDiscountedOrderValue = nonDiscountedItems.length > 0 
      ? nonDiscountedItems.reduce((sum, item) => sum + (item.grossRevenue || 0), 0) / nonDiscountedItems.length 
      : 0;
    ...
    const highDiscountItems = discountedItems.filter(item => (item.grossDiscountPercent || 0) > 25);
    const mediumDiscountItems = discountedItems.filter(item => {
      const discount = item.grossDiscountPercent || 0;
